$(function () {
    var menu = document.querySelector('.posiciotexto').offsetTop;
    $(window).scroll(function () {
        var wintop = $(window).scrollTop();
        if (wintop > menu) {
            $('header nav').css('position', 'fixed');
            $('header').css('position', 'fixed');
            $('header nav').css('bottom', '92%');
            $('header').css('bottom', '92%');
        }
        else {
